...
SimplyCountingAI::SimplyCountingAI() {
...
int SimplyCountingAI::score(Board &board, Side side) {
  ...
  if ( board.is_in_check(Utl::opposite(side)) )
    score *= 1000;

  if ( board.is_check_mate(Utl::opposite(side)) )
    score *= 1000000;
  
  return score;
}

Move SimplyCountingAI::play_turn(const Board &board, Side side) {
...
int random_f (int i) { return std::rand()%i;}

// Precondition: the vector 'board_moves' is not empty.
Board SimplyCountingAI::get_best_score(vector<Board> &board_moves, Side side) {

  std::random_shuffle ( board_moves.begin(), board_moves.end(), random_f);
